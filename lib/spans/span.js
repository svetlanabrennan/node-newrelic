...
class Span {
  ...
  static get DatastoreSpan() {
    return DatastoreSpan
  }
   ...
   *  @param {Span} params.span span to apply rules to
   ...
   *  @returns {Span|null} the span after applying the rules, or null if dropped
   */
  static applyPartialTraceRules({ span, entryPoint, partialType }) {
    const isLlmSpan = Span.isLlmSpan(span)
  ...
  static createSpan({ segment, attributes, customAttributes }) {
    let span = null
    if (HttpSpan.testSegment(segment)) {
      span = new HttpSpan(attributes, customAttributes)
    } else if (DatastoreSpan.testSegment(segment)) {
      span = new DatastoreSpan(attributes, customAttributes)
    } else {
      span = new Span(attributes, customAttributes)
    }
  ...
  /**
   * Constructs a `Span` from the given segment.
   *
   ...
   * @returns {Span} The constructed event.
   */
  static fromSegment({ segment, transaction, parentId = null, isRoot = false, inProcessSpans }) {
    ...
    const span = Span.createSpan({ segment, attributes, customAttributes })
    span.addIntrinsics({ segment, spanContext, transaction, parentId, isRoot, inProcessSpans, entryPoint })

    addSpanKind({ segment, span })
    return Span.applyPartialTraceRules({ span, entryPoint, partialType: transaction.partialType })
  }
 ...
 * @class
 */
class DatastoreSpan extends Span {
  constructor(attributes, customAttributes) {
    super(attributes, customAttributes)
