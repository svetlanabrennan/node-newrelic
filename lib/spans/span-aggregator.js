...
class SpanAggregator extends EventAggregator {
  ...
  start() {
    logger.debug('starting SpanAggregator')
    return super.start()
  }
  ...
  addSegment({ segment, transaction, parentId, isRoot }) {
    ...
    const span = Span.fromSegment({ segment, transaction, parentId, isRoot, inProcessSpans: this.inProcessSpans })
  ...
  /**
   * Reconfigure the `SpanAggregator` based on values from server
   *
