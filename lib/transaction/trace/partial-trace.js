...
class PartialTrace {
  ...
  /**
   * Called in `lib/spans/span-aggregator` which creates the spans from trace
   * Called when a transaction ends. Generates all the spans from trace.
   * This eventually calls `SpanAggregator.addSegment`. instead of enqueuing
   * span to SpanAggregator it will call `this.addSpan`
