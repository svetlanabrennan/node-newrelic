...
Trace.prototype.finalizeSpanEvents = function finalizeSpanEvents() {
  ...
  if (this.transaction.agent.spanAggregator.toString() === 'StreamingSpanAggregator') {
  ...
  for (const span of this.spans) {
    ...
    this.transaction.agent.spanAggregator.add(span, this.transaction.priority)
  }
...
/**
 * Iterates over the trace tree and generates a span for each segment.
 * @param {Node} [node] the node to process the segment and its children
 */
Trace.prototype.generateSpans = function generateSpans(node = this.segments.root) {
  ...
  const spanAggregator = this.transaction.agent.spanAggregator
